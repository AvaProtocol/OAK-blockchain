...
fn load_spec(
	...
	Ok(match id {
		...
		"oak-testnet" => Box::new(chain_spec::oak_testnet_config(para_id)),
...
impl SubstrateCli for Cli {
	...
	fn load_spec(&self, id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
		load_spec(id, self.run.parachain_id.unwrap_or(1711).into())
	}
	...
	match &cli.subcommand {
		...
		Some(Subcommand::ExportGenesisState(params)) => {
			...
			let block: Block = generate_genesis_block(&load_spec(
				...
				params.parachain_id.unwrap_or(1711).into(),
			)?)?;
		...
		None => {
			...
			runner.run_node_until_exit(|config| async move {
				...
				let id = ParaId::from(cli.run.parachain_id.or(para_id).unwrap_or(1711));
