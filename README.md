...
```bash
./target/release/oak-testnet --dev
```

Purge the development chain's state:

```bash
./target/release/oak-testnet purge-chain --dev
```
...
```shell
./target/release/oak-testnet --help
```
...
also replace the default command (`cargo build --release && ./target/release/oak-testnet --dev --ws-external`)
