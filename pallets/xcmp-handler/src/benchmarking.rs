...
benchmarks! {
	set_asset_config {
		...
		let xcm_data = XcmAssetConfig { fee_per_second: 100, instruction_weight: Weight::from_parts(1_000, 0), flow: XcmFlow::Normal };
	...
	remove_asset_config {
		...
		let versioned_location: VersionedMultiLocation = location.clone().into();
		let xcm_data =
			XcmAssetConfig { fee_per_second: 100, instruction_weight: Weight::from_parts(1_000, 0), flow: XcmFlow::Normal };
