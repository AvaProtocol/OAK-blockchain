...
pub mod pallet {
	...
	pub trait Config: frame_system::Config {
		...
		type ScheduledAllowListPallet: Contains<<Self as Config>::Call>;

		type ScheduledAllowListFrame: Contains<<Self as frame_system::Config>::Call>;
	}
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn call_now(
			...
			ensure!(T::ScheduledAllowListPallet::contains(&call), Error::<T>::CallNotAllowed);

			let encoded_call = call.encode();
			let decoded_call: <T as Config>::Call =
			Decode::decode(&mut &*encoded_call).map_err(|_| Error::<T>::BadEncodedCall)?;

			let mut dispatch_origin: T::Origin = frame_system::RawOrigin::Signed(who).into();
			dispatch_origin.add_filter(|call: &<T as frame_system::Config>::Call | { 
				T::ScheduledAllowListFrame::contains(call)
			});
