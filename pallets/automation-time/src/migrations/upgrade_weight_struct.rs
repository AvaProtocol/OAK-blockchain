	...
	fn from(action: OldAction<T>) -> Self {
		match action {
			...
			OldAction::XCMP {
				...
				xcm_asset_location,
				encoded_call,
				encoded_call_weight: Weight::from_ref_time(encoded_call_weight),
				schedule_as,
			},
...
impl<T: Config> OnRuntimeUpgrade for UpgradeWeightStruct<T> {
	fn on_runtime_upgrade() -> Weight {
		log::info!(target: "automation-time", "UpgradeWeightStruct migration");
		...
		log::info!(
			...
			"migration: UpgradeWeightStruct succesful! Migrated {} tasks.",
			migrated_tasks
		);

		<T as Config>::WeightInfo::migration_upgrade_weight_struct(migrated_tasks)
	}
...
mod test {
	...
	fn on_runtime_upgrade() {
		new_test_ext(0).execute_with(|| {
			...
			UpgradeWeightStruct::<Test>::on_runtime_upgrade();
				...
				TaskOf::<Test> {
					...
					action: ActionOf::<Test>::XCMP {
						...
						xcm_asset_location: MultiLocation::new(1, X1(Parachain(para_id.into())))
							.into(),
						encoded_call: vec![0u8],
						encoded_call_weight: Weight::from_ref_time(10),
						schedule_as,
					},
				}
			);
