...
use super::*;
	...
	impl<T: Config> Pallet<T> {
				...
				let action_weight = match AccountTasks::<T>::get(account_id.clone(), task_id) {
					...
					Some(mut task) => {
						let task_action_weight = match task.action.clone() {
							...
							Action::Generic { encoded_call } => Self::run_generic_action(task.owner_id.clone(), encoded_call, *task_id),
						};
		...
		pub fn run_auto_compound_delegated_stake_task(
			...
			let reserved_funds =
				account_minimum.saturating_add(Self::calculate_execution_fee(&task.action, 1).unwrap());
		...
		fn run_generic_action(caller: AccountOf<T>, encoded_call: Vec<u8>, task_id: TaskId<T>) -> Weight {
			match <T as Config>::Call::decode(&mut &*encoded_call) {
				Ok(scheduled_call) => {
					let dispatch_origin: T::Origin = frame_system::RawOrigin::Signed(caller.clone()).into();
					// let filtered_origin = dispatch_origin.add_filter();


					let call_weight = scheduled_call.get_dispatch_info().weight;
					let (maybe_actual_call_weight, result) = match scheduled_call.dispatch(dispatch_origin) {
				...
				Err(_) => {
					...
					Self::deposit_event(Event::CallCannotBeDecoded {
						who: caller,
						task_id: task_id,
					});
					// TODO: fix weight
		...
		pub fn calculate_execution_fee(action: &ActionOf<T>, executions: u32) -> Result<BalanceOf<T>, DispatchError>  {
			let action_weight = match action {
				...
				Action::AutoCompoundDelegatedStake { .. } =>
					<T as Config>::WeightInfo::run_auto_compound_delegated_stake_task(),
				Action::Generic { encoded_call } => {
					let scheduled_call: <T as Config>::Call =
						Decode::decode(&mut &**encoded_call).map_err(|_| Error::<T>::CallCannotBeDecoded)?;
