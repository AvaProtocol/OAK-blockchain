use crate::*;
use frame_support::{
	traits::OnRuntimeUpgrade, Blake2_128Concat, BoundedVec, Twox64Concat, WeakBoundedVec,
};
...
impl OnRuntimeUpgrade for MigrateTokensCurrencyId {
	...
	fn post_upgrade() -> Result<(), &'static str> {
		...
		let new_total_issuance = orml_tokens::TotalIssuance::<Runtime>::iter().collect::<Vec<_>>();
